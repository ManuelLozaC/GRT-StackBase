...
async function loadUsers() {
    ...
    try {
        ...
        state.items = (response.data.datos ?? []).map((user) => ({
            ...user,
...
<template>
    <div class="space-y-6">
        <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    ...
                    <p class="text-slate-600 max-w-3xl">Desde aqui puedes validar multi-rol por usuario e impersonacion administrativa sobre la organizacion activa.</p>
                </div>
        ...
        <div v-else class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <StateEmpty v-if="!hasUsers" title="No hay usuarios en la organizacion activa" description="Cuando existan miembros dentro del tenant actual, aqui podras gestionar sus roles e impersonarlos para soporte." />

            <DataTable v-else :value="users" dataKey="id">
                <Column field="name" header="Usuario" style="min-width: 15rem">
