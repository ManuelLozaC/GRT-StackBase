...
export const layoutPresetOptions = Object.keys(layoutPresets);

export const layoutMenuModeOptions = [
    { label: 'Lateral fijo', value: 'static' },
    { label: 'Lateral overlay', value: 'overlay' }
];
...
export function findPrimaryColor(name) {
    return primaryColorOptions.find((color) => color.name === name) ?? primaryColorOptions.find((color) => color.name === 'emerald');
}

export function findSurfacePalette(name) {
    return surfacePaletteOptions.find((surface) => surface.name === name) ?? surfacePaletteOptions.find((surface) => surface.name === 'slate');
}

export function buildPresetExt(primaryName) {
    const color = findPrimaryColor(primaryName);
